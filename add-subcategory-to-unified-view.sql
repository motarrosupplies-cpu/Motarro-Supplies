-- Add subcategory column to all_products_unified view
-- This migration updates the view to include subcategory from all product tables

-- Drop existing view
DROP VIEW IF EXISTS public.all_products_unified;

-- Recreate view with subcategory column
...
CREATE INDEX IF NOT EXISTS idx_simple_products_subcategory ON public.simple_products(subcategory) WHERE subcategory IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_color_only_products_subcategory ON public.color_only_products(subcategory) WHERE subcategory IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_size_only_products_subcategory ON public.size_only_products(subcategory) WHERE subcategory IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_full_variant_products_subcategory ON public.full_variant_products(subcategory) WHERE subcategory IS NOT NULL;
