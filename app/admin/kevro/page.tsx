...
import type { KevroMarkupRule } from "@/lib/kevro/markup";
...
export default function AdminKevroPage() {
  ...
  const [status, setStatus] = useState<{
    productCount: number;
    lastSync: { status: string; product_count: number; finished_at: string | null; error_message: string | null } | null;
  } | null>(null);

  async function loadRules() {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/kevro/markup", {
        headers: await getAdminAuthHeaders(),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Failed to load rules");
      setRules(
        (data.rules as KevroMarkupRule[]).map((rule) => ({
          id: rule.id,
  ...
  async function loadStatus() {
    try {
      const response = await fetch("/api/admin/kevro/status", {
        headers: await getAdminAuthHeaders(),
      });
      const data = await response.json();
      if (response.ok) {
  ...
  async function saveRules() {
    ...
    try {
      ...
      toast({ title: "Saved", description: "Kevro markup rules updated." });
    } catch (error) {
      toast({
        title: "Error",
  ...
  async function syncFeed() {
    setSyncing(true);
    try {
      const response = await fetch("/api/admin/kevro/sync", {
        method: "POST",
        headers: await getAdminAuthHeaders(),
      });
      const data = await response.json();
      ...
      toast({
        title: "Sync complete",
        description: `${data.productCount} products synced. View them at /branded-catalog`,
      });
      await loadStatus();
    } catch (error) {
      toast({
  ...
  async function deleteRule(index: number) {
    ...
    if (rule.id) {
      const response = await fetch(`/api/admin/kevro/markup?id=${rule.id}`, {
        method: "DELETE",
        headers: await getAdminAuthHeaders(),
      });
      if (!response.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Kevro Catalog</h1>
          <p className="text-muted-foreground">
            Manage supplier feed sync, markup rules, and branded catalog pricing.
          </p>
        </div>
      ...
      {status && (
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <p>
              Storefront:{" "}
              <a href="/branded-catalog" className="text-primary underline">
                www.motarro.co.za/branded-catalog
              </a>
            </p>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            ...
            `KEVRO_PRICE_MARKUP_PERCENT` env fallback.
          </p>
      ...
      <Card>
        ...
        <CardContent className="text-sm text-muted-foreground space-y-2">
          ...
          <code className="block rounded bg-muted p-3 text-xs">
            GET /api/cron/sync-kevro-feed
            <br />
            Authorization: Bearer CRON_SECRET
          </code>
          <p>
            Run the SQL in `supabase-kevro-schema.sql` in Supabase before the first sync.
          </p>
        </CardContent>
      </Card>
    </div>
