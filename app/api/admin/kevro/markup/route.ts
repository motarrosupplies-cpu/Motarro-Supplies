...
import {
  clearKevroMarkupCache,
  getKevroMarkupRules,
} from "@/lib/kevro/markup";
...
export async function PUT(request: Request) {
  ...
  for (const rule of rules) {
    ...
    // Update by id when possible — upsert on (rule_type, rule_value) fails for
    // global rules because PostgreSQL treats NULL as distinct in UNIQUE constraints.
    if (rule.id) {
  ...
  clearKevroMarkupCache();
  const updated = await getKevroMarkupRules();
