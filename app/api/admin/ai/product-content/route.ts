import { NextResponse } from "next/server";
import { verifyAdminRequest } from "@/lib/auth/verifyAdminApi";
import { isXaiConfigured } from "@/lib/xai/config";
import { generateProductContent } from "@/lib/xai/product-content";
import { XaiClientError } from "@/lib/xai/client";

export const maxDuration = 60;
export const dynamic = "force-dynamic";

export async function POST(request: Request) {
  let body: Record<string, unknown>;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: "Invalid JSON body" }, { status: 400 });
  }

  const auth = await verifyAdminRequest(
    request,
    typeof body.accessToken === "string" ? body.accessToken : null
  );
  if (!auth.ok) {
    return NextResponse.json(
      { error: auth.error, code: auth.code },
      { status: auth.status }
    );
  }

  if (!isXaiConfigured()) {
    return NextResponse.json(
      { error: "XAI_API_KEY is not configured on the server", code: "config" },
      { status: 503 }
    );
  }

  try {
    const content = await generateProductContent({
      name: String(body.name || "").trim(),
      category: body.category ?? null,
      subcategory: body.subcategory ?? null,
      price:
        body.price === null || body.price === undefined
          ? null
          : Number(body.price),
      sku: body.sku ?? null,
      colors: Array.isArray(body.colors) ? body.colors : [],
      sizes: Array.isArray(body.sizes) ? body.sizes : [],
      existingDescription: body.existingDescription ?? null,
    });

    if (!content.description) {
      return NextResponse.json(
        { error: "Generated content was empty" },
        { status: 502 }
      );
    }

    return NextResponse.json({ content });
  } catch (error) {
    if (error instanceof XaiClientError) {
      return NextResponse.json(
        { error: error.message, code: "xai" },
        { status: error.status }
      );
    }

    console.error("[admin/ai/product-content]", error);
    return NextResponse.json(
      { error: "Failed to generate product content" },
      { status: 500 }
    );
  }
}
